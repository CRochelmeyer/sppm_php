...
$to = $_SESSION['rdateto'];
$pid = $_SESSION['report_product_id'];
$sku = $_SESSION['report_product_sku'];
$nm = $_SESSION['report_product_name'];
...
$sql="SELECT ps.product_id, ps.quantity_sold, s.sale_id, s.time_created, s.sale_amount, p.quantity, p.sku, p.name, p.type 
    ...
    WHERE s.time_created BETWEEN '$from' AND '$to' AND ps.product_id LIKE '$pid' OR p.sku LIKE '$sku' OR p.name LIKE '$nm' ;";
